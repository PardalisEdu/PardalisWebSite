<script>
    ...
    function generateOrder() {
        ...
        const tacoType = Object.values(tacoTypes)[Math.floor(Math.random() * Object.values(tacoTypes).length)];
    ...
    function deliverOrder() {
        ...
        const isCorrect = preparedTacos.every(taco => 
...
<div class="min-h-screen bg-gradient-to-b from-orange-100 to-orange-200 pt-24 px-4">
    <div class="max-w-4xl mx-auto">
                ...
                <div class="flex items-center gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600">
                            I want {currentOrder.quantity} {currentOrder.type.english} taco{currentOrder.quantity > 1 ? 's' : ''} 
                            {currentOrder.withVegetables ? 'with' : 'without'} vegetables,
                            {currentOrder.withSalsa ? 'with' : 'without'} salsa
                        </p>
                        <p class="text-sm text-gray-500">
                            ({currentOrder.quantity} taco{currentOrder.quantity > 1 ? 's' : ''} de {currentOrder.type.name} 
                    ...
                    <div class="flex gap-2 flex-wrap">
                        {#each preparedTacos as taco, i}
                            <div class="bg-orange-100 p-2 rounded-lg flex items-center gap-1">
                                <span>{tacoTypes[Object.keys(tacoTypes).find(k => tacoTypes[k].name === taco.type)].emoji}</span>
        ...
        {#if message}
            <div class="fixed bottom-4 right-4 p-4 rounded-lg shadow-lg" class:bg-green-500={messageType === 'success'} class:bg-red-500={messageType === 'error'} class:bg-blue-500={messageType === 'info'}>
